...
export default function BacklogPageContent({ projectKey, issueUrlPrefix }: { projectKey: string; issueUrlPrefix?: string }): React.JSX.Element {
 ...
 const filterIssues = (issueList: ITaskData[]): ITaskData[] =>  issueList.filter(issue =>
    ...
    <div className="flex flex-col h-full bg-surface text-content">
      ...
      <div className="px-4 pt-4 pb-4">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:justify-between sm:items-center mb-4">
          <h1 className="text-2xl font-semibold text-content">Backlog</h1>
          {canCreateTask && (
            <button
 onClick={() => setIsTaskModalOpen(true)}
 className="w-full sm:w-auto bg-primary hover:bg-primary-hover text-primary-content px-4 py-2 rounded-md font-medium text-sm transition-colors cursor-pointer text-center"
            >
 Create Task
            </button>
          )}
        ...
        <div className="flex items-center gap-4">
          <div className="relative w-full sm:w-64">
            <input
 type="text" placeholder="Search backlog" value={searchQuery}
 ...
 issueUrlPrefix={issueUrlPrefix}
          ...
          <BacklogList
 listId="backlog" title="Backlog" issues={filterIssues(backlogIssues)}
 projectKey={projectKey}
