        ...
        >
            ...
            <div className="flex items-center justify-between mb-3 h-8 group sticky top-0 bg-[#0E1015] z-10">
                <div className="flex items-center gap-2">
                    <col.icon className={cn("w-3.5 h-3.5", col.iconColor)} />
                ...
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button className="h-6 w-6 flex items-center justify-center rounded hover:bg-white/5 text-[#8A8F98] transition-colors focus:outline-none"><Plus className="w-3.5 h-3.5" /></button>
                    <button className="h-6 w-6 flex items-center justify-center rounded hover:bg-white/5 text-[#8A8F98] transition-colors focus:outline-none"><MoreHorizontal className="w-3.5 h-3.5" /></button>
                </div>
            </div>

            {/* Cards Container */}
            <div className="flex-1 flex flex-col gap-2 overflow-y-auto pb-8 pr-1 custom-scrollbar">
                {col.items.map((task: Task, itemIndex) => (
                    <motion.div
                        key={task._id}
                        initial={{ opacity: 0, y: 10 }}
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ delay: (index * 0.05) + (itemIndex * 0.03), duration: 0.2 }}
                        className="group bg-[#1C1D22] border border-white/4 rounded-lg p-3 cursor-pointer hover:bg-[#232429] hover:border-white/8 transition-colors flex flex-col gap-2 relative shadow-sm"
                    >
                        ...
                        <div className="flex items-center gap-1.5 ml-5 mt-1">
                            {task.priority === 'high' && (
                                <div className="flex items-center gap-1 px-1.5 py-0.5 rounded-sm border border-white/6 bg-white/2 text-[#8A8F98]">
                ...
                <button className="flex items-center gap-2 py-1.5 px-2 mt-1 text-[13px] text-[#8A8F98] hover:text-[#E8E8FD] hover:bg-white/4 rounded-md transition-colors focus:outline-none">
