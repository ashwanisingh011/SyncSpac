...
export default function WorkspaceSidebar() {
  ...
  return (
    <aside
      className={clsx(
        'shrink-0 border-r border-line bg-surface-sunken flex flex-col transition-all duration-300',
        collapsed ? 'w-20' : 'w-64'
      )}
    >
      ...
      <div className="relative border-b border-line">
        <button
          ...
          className="absolute -right-3 top-5 z-50 h-6 w-6 rounded-full border bg-surface shadow flex items-center justify-center"
        >
          {collapsed ? (
        ...
        <button
          ...
          className={clsx(
            'w-full p-4 hover:bg-surface-hover transition-colors',
            collapsed
              ? 'flex justify-center'
        ...
        >
          {currentOrg?.logoUrl ? (
            ...
            <div className="w-9 h-9 rounded-lg bg-primary text-primary-content flex items-center justify-center font-bold text-sm shrink-0">
              {initials}
            </div>
          )}
          ...
          {!collapsed && (
            <>
              ...
              <ChevronDown
                className={clsx(
                  'w-4 h-4 text-content-tertiary shrink-0 transition-transform',
        ...
        {!collapsed && showOrgPicker && (
          <div className="absolute top-full left-0 right-0 z-30 bg-surface border border-line shadow-lg">
            {currentOrg && (
              <div className="px-4 py-2 border-b border-line">
                <p className="text-xs text-content-tertiary mb-1">Your role</p>
                <OrgRoleBadge
            ...
            {organizations.map((org) => (
              ...
              >
                {org.logoUrl ? (
                  ...
                  <div className="w-7 h-7 rounded-md bg-primary text-primary-content flex items-center justify-center font-bold text-xs shrink-0">
            ...
            {hasPermission('create_workspace') && (
              <div className="border-t border-line p-2">
                <Link
                  ...
                  className="flex items-center gap-2 w-full px-3 py-2 text-xs text-content-tertiary hover:text-primary hover:bg-primary-subtle rounded-lg transition-colors"
                >
      ...
      <div className="p-3 border-t border-line">
        <Link
          ...
          className={clsx(
            'flex items-center w-full px-3 py-2 text-sm text-content-tertiary hover:bg-surface-hover hover:text-content rounded-md transition-colors',
