...
export const IssueTypeIcon = ({ type, className }: IssueTypeIconProps): React.JSX.Element => {
  ...
  switch (normalizedType) {
    ...
    case 'bug':
      return <div className={clsx("w-5 h-5 rounded-sm bg-danger flex items-center justify-center", className)}><Bug className="w-3 h-3 text-white" /></div>;
    case 'subtask':
    ...
    case 'task':
    default:
      return <div className={clsx("w-5 h-5 rounded-sm bg-primary flex items-center justify-center", className)}><CheckSquare className="w-3 h-3 text-white" /></div>;
  }
};
