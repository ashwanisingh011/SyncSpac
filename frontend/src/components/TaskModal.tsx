...
const TaskModal = ({ isOpen, onClose, taskToEdit, refreshTasks }: TaskModalProps): React.JSX.Element | null => {
 ...
 return (
    <div className="fixed inset-0 bg-black/45 flex items-center justify-center z-50 p-4">
      ...
      <div className="bg-surface rounded-lg shadow-2xl w-full max-w-md border border-line">
        <div className="flex items-center justify-between px-4 py-4 border-b border-line">
          ...
          <button
 onClick={onClose}
 className="text-content-tertiary hover:text-content text-2xl leading-none cursor-pointer" aria-label="Close"
          >
            &times;
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
          {error && (
            <div className="bg-danger/10 border border-danger/25 text-danger text-sm px-3 py-2 rounded-lg">
              {error}
            </div>
          )}

          <div>
            <label className="block text-sm font-medium text-content-secondary mb-1">Task Type</label>
            <select
 ...
 onChange={handleChange}
 className="w-full border border-line-strong rounded px-3 py-2 text-sm bg-surface text-content placeholder:text-content-tertiary focus:outline-none focus:ring-2 focus:ring-primary/30" placeholder="Task title"
            />
          </div>
 ...
 onChange={handleChange}
 rows={3}
 className="w-full border border-line-strong rounded px-3 py-2 text-sm bg-surface text-content placeholder:text-content-tertiary focus:outline-none focus:ring-2 focus:ring-primary/30 resize-none" placeholder="Optional description"
            />
          </div>

          {isSelfAssignOnly ? (
            <div>
              <label className="block text-sm font-medium text-content-secondary mb-1">Assignee</label>
              <input
 type="text" disabled
 value={taskToEdit ? (taskToEdit.assignedTo === (user?.id || user?._id) ? "Assigned to Me" : "Assigned to Other") : "Assigned to Me"}
 className="w-full border border-line-strong rounded px-3 py-2 text-sm bg-surface-hover text-content-tertiary focus:outline-none"
              />
            </div>
          ) : (
            <div>
 ...
 className="w-full border border-line-strong rounded px-3 py-2 text-sm bg-surface text-content focus:outline-none focus:ring-2 focus:ring-primary/30"
            ...
            <div>
              <label className="block text-sm font-medium text-content-secondary mb-1">Original Estimate (min)</label>
              <input
 type="number" name="estimatedTime" min="0" value={form.estimatedTime}
 onChange={handleChange}
 placeholder="e.g. 180" className="w-full border border-line-strong rounded px-3 py-2 text-sm bg-surface text-content focus:outline-none focus:ring-2 focus:ring-primary/30"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-content-secondary mb-1">Sprint</label>
              <select
 name="sprintId" value={form.sprintId}
 onChange={handleChange}
 className="w-full border border-line-strong rounded px-3 py-2 text-sm bg-surface text-content focus:outline-none focus:ring-2 focus:ring-primary/30"
              >
            ...
            <div>
              <label className="block text-sm font-medium text-content-secondary mb-1">Story Points</label>
              <input
 type="number" name="storyPoints" min="0" value={form.storyPoints}
 onChange={handleChange}
 placeholder="e.g. 5" className="w-full border border-line-strong rounded px-3 py-2 text-sm bg-surface text-content focus:outline-none focus:ring-2 focus:ring-primary/30"
              />
            </div>
          </div>
 ...
 className="px-4 py-2 text-sm text-content-secondary border border-line-strong rounded hover:bg-surface-hover cursor-pointer"
            >
 Cancel
            </button>
            <button
 type="submit" disabled={loading}
 className="px-4 py-2 text-sm text-primary-content bg-primary rounded hover:bg-primary-hover disabled:opacity-60 cursor-pointer"
            >
